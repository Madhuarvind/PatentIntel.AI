        ...
        for attempt in range(1, max_retries + 1):
            ...
            try:
                ...
                if http_err.code in (503, 504, 429) and attempt < max_retries:
                    print(f"   [WAIT] Hugging Face model is loading into GPU memory (HTTP Status {http_err.code}).")
                    print(f"          Waiting 15 seconds for Hugging Face GPU warm-up... (Retry {attempt}/{max_retries})")
                    time.sleep(15)
                else:
                    raise http_err
            except urllib.error.URLError as url_err:
                if attempt < max_retries:
                    print(f"   [RETRY] Network/DNS Notice: {url_err.reason}")
                    print(f"           Retrying connection in 10 seconds... (Attempt {attempt}/{max_retries})")
                    time.sleep(10)
                else:
                    raise url_err
