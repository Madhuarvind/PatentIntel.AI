...
export const PatentWorkspaceView: React.FC = () => {
  ...
  const currentPatent = mockPatents.find(p => p.id === selectedPatent) || mockPatents[0];
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div>
          ...
          <p style={{ fontSize: '0.88rem', color: 'var(--text-muted)' }}>
            Upload patent PDFs (USPTO / EPO / WIPO), extract structured metadata, independent claims, and dependent clauses.
          </p>
        </div>
        ...
        <div style={{ display: 'flex', gap: '10px' }}>
          ...
          >
            <FileText size={16} /> Patent Library ({mockPatents.length})
          </button>
          <button 
            className={activeTab === 'upload' ? 'btn-primary' : 'btn-secondary'}
            onClick={() => setActiveTab('upload')}
          >
            <Plus size={16} /> Upload New Patent PDF
          </button>
        </div>
      </div>
          ...
          <div className="glass-panel" style={{ padding: '24px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', borderBottom: '1px solid var(--border-color)', paddingBottom: '16px', marginBottom: '20px' }}>
              <div>
                ...
                <div style={{ fontSize: '0.84rem', color: 'var(--text-muted)', display: 'flex', gap: '16px' }}>
            ...
            <div>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '12px' }}>
                <h3 style={{ fontSize: '0.95rem', fontWeight: 700, color: 'var(--text-main)', margin: 0 }}>
                  Extracted Claims (Claim 1 Independent)
                </h3>
                <span style={{ fontSize: '0.78rem', color: 'var(--accent-cyan)' }}>18 Total Claims Extracted</span>
              </div>
              ...
              <div style={{ background: 'var(--bg-card-solid)', border: '1px solid rgba(0, 242, 254, 0.25)', padding: '16px', borderRadius: '12px' }}>
                ...
                <p style={{ fontSize: '0.9rem', color: 'var(--text-main)', lineHeight: '1.65', fontFamily: 'var(--font-sans)' }}>
                  "1. A smart autonomous vehicle collision warning apparatus comprising: 
                  an optical camera sensor configured to capture visual video frames of an external roadway environment; 
                  a deep neural network processor coupled to said optical camera sensor configured to detect dynamic obstacles and compute collision probability metrics; 
                  a real-time warning controller configured to generate a collision hazard alert signal when said computed collision probability exceeds a predefined threshold; and 
                  a visual display interface positioned within a vehicle cockpit to display said warning signal."
                </p>
              </div>
            </div>
          </div>
